...
%%% XXX: state of codec shouldn't leave pg_sock process. If you need to
...
-callback init(any(), pg_sock:pg_sock()) -> codec_state().

%% List of supported type names
-callback names() -> [pg_datatypes:type_name()].

%% Encode Erlang representation to PG binary
%% Called for each parameter, binary protocol (equery)
-callback encode(Cell :: any(), pg_datatypes:type_name(), codec_state()) -> iodata().

%% Decode PG binary to erlang representation
%% Called for each cell in each row, binary protocol (equery)
-callback decode(Cell :: binary(), pg_datatypes:type_name(), codec_state()) -> any().

%% Decode PG string representation (text protocol) to erlang term.
%% Called for each cell in each row, text protocol (squery)
-callback decode_text(Cell :: binary(), pg_datatypes:type_name(), codec_state()) ->
    any().

%% ==========
-type codec_entry() :: {pg_datatypes:type_name(),
                        Mod :: codec_mod(),
                        CallbackState :: any()}.

-spec init_mods([{codec_mod(), any()}], pg_sock:pg_sock()) ->
...
-spec encode(codec_mod(), any(), pg_datatypes:type_name(), codec_state()) -> iodata().
encode(Mod, Cell, TypeName, CodecState) ->
    Mod:encode(Cell, TypeName, CodecState).

-spec decode(codec_mod(), binary(), pg_datatypes:type_name(), codec_state()) -> any().
decode(Mod, Cell, TypeName, CodecState) ->
    Mod:decode(Cell, TypeName, CodecState).

-spec decode_text(codec_mod(), binary(), pg_datatypes:type_name(), codec_state()) -> any().
